...
with open('./calibration.json') as f:
    ...
    code.append('\tstatic constexpr std::array<precalculated,{}> setpoints{{'.format(len(datasets)))
    names = []
    for element in datasets:
        first = min(element['points'], key=lambda e: e['index'])
        last = max(element['points'], key=lambda e: e['index'])
        m = (math.log10(last['y']) - math.log10(first['y'])) / (math.log10(last['x']) - math.log10(first['x']))
        b = statistics.mean([math.log10(point['y']) - (m * math.log10(point['x'])) for point in element['points']])
